...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Decoder
        x12 = self.dec_conv6(x11)
        x13 = self.up5(x12)
        x14 = self.dec_conv5(x13 + x9)
        x15 = self.up4(x14)
        x16 = self.dec_conv4(x15 + x7)
        x17 = self.up3(x16)
        x18 = self.dec_conv3(x17 + x5)
        x19 = self.up2(x18)
        x20 = self.dec_conv2(x19 + x3)
        x21 = self.up1(x20)
        x22 = self.dec_conv1(x21 + x1)

        # Output
        x23 = self.out_conv(x22)
        return x23
...
def main():
    ...
    img_height = 384
    img_width = 256
    epochs = 48  # Move this line outside the loop
    ...
    transform = transforms.Compose([
        ...
        #transforms.ColorJitter(brightness=0.1, contrast=0.1, saturation=0.1, hue=0.1),  # Add color jittering
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=16, gamma=0.2)
